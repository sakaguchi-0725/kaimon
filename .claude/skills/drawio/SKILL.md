---
name: drawio
description: 図やダイアグラムの作成・生成・描画を依頼されたときに使用する。フローチャート、アーキテクチャ図、ER図、シーケンス図、クラス図、ネットワーク図、モックアップ、ワイヤーフレーム、UI設計図など。draw.io、drawio、.drawioファイル、PNG/SVG/PDFへのエクスポートに言及された場合も使用する。
---
...
- 図やダイアグラムの作成・生成・描画を依頼されたとき
- draw.io、.drawio ファイル、PNG/SVG/PDF エクスポートに言及されたとき
- フローチャート、ER図、シーケンス図、アーキテクチャ図などの作成を求められたとき
...
|---------|-------|----------|
| draw.io CLI not found | Desktop app not installed or not on PATH | Keep the `.drawio` file and tell the user to install the draw.io desktop app, or open it manually |
...
**Remember**: .drawio ファイルは mxGraphModel XML で直接生成する。Mermaid は使わない。
